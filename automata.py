...
def postprocess(frame, outs):
    ...
    for i in indices:
        ...
        newfile=tf.image.crop_to_bounding_box(
                 frame,
                 top,
                 left,
                 height,
...
while cv.waitKey(1) < 0:
    ...
    newfile=postprocess(frame, outs)
    ...
    if (args.image):
        cv.imwrite(outputFile, newfile.astype(np.uint8));
        ...
        text = pytesseract.image_to_string(outputFile, config=custom_config)    	#Data is stored in CSV file
